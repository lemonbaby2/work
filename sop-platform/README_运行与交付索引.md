# 宁波零部件 SOP 智能分析决策平台（关键帧标注与三模型对比版）
...
2026-08-20 又接入两段 PCB 人工插装 4K 视频：`DJI_20260819145537_0264_D.MP4` 与
`DJI_20260819151908_0265_D.MP4`。网页代理为 720p/30 FPS；标注采用固定工位区域、
YOLOE 开放词汇候选、稳定段每 15–30 帧关键帧、运动段每 3–5 帧关键帧和线性插值，
无需逐帧重复画框。

- 0264：945.57 秒，28,367 个代理帧，1,595 张目标视频关键帧；
- 0265：659.27 秒，19,778 个代理帧，1,182 张目标视频关键帧；
- 0264 数据集连同独立验证/测试共 3,292 张图、26,292 个自动候选；
- 0265 数据集连同独立验证/测试共 2,879 张图、25,531 个自动候选；
- 训练/验证/测试按完整源视频隔离，禁止相邻帧随机切分；
- 客户可比较 0264 单视频、0265 单视频、0264+0265 联合三个 YOLO26N 候选；
- 只有权重和独立测试报告均存在的模型可以在网页选择，每次选择写入审计日志；
- 全部自动候选和模型保持 `HOLD`，NG、稀有类、遮挡、方向变化和工序边界须人工复核。

两台 Windows 的离线交付目录统一为 `D:\soplzp0820_20260820`。公开 GitHub 分支为
`https://github.com/lemonbaby2/work/tree/soplzp0820`；原视频、标注图片、SQLite 和权重不上传公开仓库。
