...
def network_camera_profiles() -> list[dict]:
    if not NETWORK_CAMERA_PATH.exists():
        return []
    try:
        value = json.loads(NETWORK_CAMERA_PATH.read_text(encoding="utf-8"))
        return value.get("cameras", []) if isinstance(value, dict) else []
    ...
    def start(self) -> None:
        with self._lock:
            ...
            try:
                ...
                if self.camera_id == 2 and not self.network_profile.get("rtsp_path_confirmed", False):
                    raise RuntimeError("第三摄像头身份已绑定，但 RTSP 路径尚未确认；请在网页绑定区填写实际 RTSP 地址并测试通过")
        ...
        try:
            ...
            if path == "/api/network-cameras/test":
                ...
                try:
                    ...
                    self.send_json({"ok": False, "message": "RTSP 测试超时：设备在线但地址未返回视频流，请从摄像头 App 复制完整 RTSP 地址", "rtsp_url": rtsp_url}, 422)
                    return
                except OSError as exc:
                    self.send_json({"ok": False, "message": f"本机无法执行 ffprobe：{exc}", "rtsp_url": rtsp_url}, 503)
                    return
                if result.returncode != 0:
                    message = (result.stderr or "RTSP 无视频流").strip().splitlines()[-1]
                    self.send_json({"ok": False, "message": message, "rtsp_url": rtsp_url}, 422)
                    return
