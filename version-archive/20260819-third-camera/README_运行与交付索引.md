...
第三摄像头已登记为平台 `camera 2`：IP `192.168.1.135`，MAC `c4:3c:b0:be:40:e8`。现场监控页新增“网络摄像头 3 绑定”，可填写厂家完整 RTSP 地址、测试流并保存。设备 554 端口和 `Hopeway RealServer/V1.0` 服务已经识别，但默认根路径返回 400，因此 `rtsp://192.168.1.135:554/` 只是占位地址，不能标记为可用视频流。
