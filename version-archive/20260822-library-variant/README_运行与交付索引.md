...
“现场监控”页面已加入实时相机检测区域。点击“启动实时检测”后，后端按需打开 `/dev/video0`，使用 `models/yolo11n.pt` 在 DGX Spark `cuda:0` 推理，并通过 `/api/camera/mjpeg` 向浏览器发送带框画面；`/api/camera/status` 返回 FPS、分辨率、推理耗时和当前目标数。
...
页面地址：`http://127.0.0.1:8096`。同一时刻只能有一个进程独占 USB 摄像头；若页面提示无法打开相机，先用 `fuser -v /dev/video0` 查找正在占用的采集进程。

双相机配置使用 `SOP_CAMERA_SOURCE_0` 和 `SOP_CAMERA_SOURCE_1`，网页点击“启动双相机检测”后两路同时启动，下拉框切换显示画面：

```bash
SOP_CAMERA_SOURCE_0=0 SOP_CAMERA_SOURCE_1=1 ./启动SOP平台_DGX.sh
```
...
- `启动实时检测`：按当前选择打开摄像头，使用低缓存和固定 1280×720 采集，页面通过 MJPEG 只保留最新帧以降低延迟。
...
第三摄像头已登记为平台 `camera 2`：IP `192.168.1.135`，MAC `c4:3c:b0:be:40:e8`。现场监控页新增“网络摄像头 3 绑定”，可填写厂家完整 RTSP 地址、测试流并保存。`config/network_cameras.json` 中的根地址只是占位地址，不能直接启动。服务端会先检查 554 端口和 RTSP 流，网络不可达时会返回明确的 DHCP/局域网排查信息；设备恢复在线后也可用 `scripts/probe_network_camera.py` 独立验证。

固定方式建议：在路由器 `192.168.1.1` 的 DHCP 静态租约中，将 MAC `c4:3c:b0:be:40:e8` 固定分配为 `192.168.1.135`；若摄像头 App 使用手工静态 IP，应从 DHCP 地址池排除 `.135`。完整操作见 `docs/10_第三网络摄像头绑定与固定IP.md`。

Windows Advanced Installer 构建材料位于 `installer/windows`。当前 DGX/Ubuntu 无法直接运行 Windows Advanced Installer，因此交付的是可在 Windows 上执行的构建脚本、启动脚本、依赖清单和操作说明；不能把未实际构建的文件伪称为 EXE。Ubuntu 桌面启动器位于 `installer/linux`。
...
```bash
export LABEL_STUDIO_IMAGE='local/label-studio:1.23.0'
export LABEL_STUDIO_IMAGE_FILE='./deploy/label-studio/label-studio-image.tar'
./scripts/start_label_studio.sh
```
...
```bash
./scripts/cache_label_studio_image.sh
```

该脚本将 Label Studio 1.23.0 固定为 `local/label-studio:1.23.0` 并导出到 `deploy/label-studio/label-studio-image.tar`。之后启动脚本只从本机镜像或该归档加载，不会主动访问 Docker Hub。Compose 同时关闭了 Sentry 上报、版本检查和转换器资源自动下载。完全离线的 Ubuntu 机器只需把这个归档文件带到同一路径即可。内网环境也可以把 `LABEL_STUDIO_SOURCE_IMAGE` 或 `LABEL_STUDIO_IMAGE` 指向自己的镜像仓库。

本机 Ubuntu 上推荐的使用方式是：

1. 用 `./启动本地标注工作台.sh` 同时拉起本平台和 Label Studio。
2. 在网页里用“数据标注”页做轻量修框、审核和导出。
3. 需要更完整的项目管理、标签体系和多人协作时，再切到 Label Studio。

CVAT 适合更重的工业标注中心，但本地单机场景下维护成本更高，不是首选。
